 ...
 */
void RadioInit( RadioEvents_t *events, DioIrqHandler dioIrq );
 ...
 */
void RadioSend( uint8_t *buffer, uint8_t size, uint32_t timeout );
 ...
 */
//void RadioOnDioIrq( nrfx_gpiote_pin_t pin, nrf_gpiote_polarity_t action );
...
void RadioSend( uint8_t *buffer, uint8_t size, uint32_t timeout )
{
    ...
    SX126xSendPayload( buffer, size, timeout << 6 );
}
